#Model M2: DRCC Co-optimization with McCormick Relaxation + Chebyshev Inequality
using JuMP, Distributions, LinearAlgebra, DataFrames, Gurobi

#Prepare and load data
include("CS1_24bus/data_load_script_PSCC.jl")
...
function unidir_DRCC_McCormick_SOCP_EL_NG()
    ...
    #EL Variables
    @variable(m, p[1:Np, 1:Nt] >= 0)        #Production from power generators
    ...
    #NG Variables
    @variable(m, g[1:Ng, 1:Nt])             #Gas production from gas producers
    ...
    #@constraint(m, [i=1:Np, t=1:Nt], α[i,t]* + 2*p[i,t]*gen_data[i].p̅ <= p[i,t]*p[i,t] + (gen_data[i].p̅)^2)
    ...
    @constraint(m, q_nonneg[pl=1:Nng_line, t=1:Nt], q[pl,t] >= 0)
    @constraint(m, qin_nonneg[pl=1:Nng_line, t=1:Nt], q_in[pl,t] >= 0)
    @constraint(m, qout_nonneg[pl=1:Nng_line, t=1:Nt], q_out[pl,t] >= 0)
    ...
    #@constraint(m, wm_soc[pl=1:Nng_line, t=1:Nt], [ngLine_data[pl].K_mu*pr[ngLine_data[pl].ng_f,t], q[pl,t], ngLine_data[pl].K_mu*pr[ngLine_data[pl].ng_t]] in SecondOrderCone())
    ...
    #uncomment if using non-convex solver such as Ipopt, Baron
    #@constraint(m, wm_nonconv[pl=1:Nng_line, t=1:Nt],q[pl,t]^2 <= ngLine_data[pl].K_mu^2*(pr[ngLine_data[pl].ng_f,t]^2 -  pr[ngLine_data[pl].ng_t,t]^2))
    ...
    for pl=1:Nng_line
        for t̂=1:Nt
            if t̂ == 1      #First Hour
                #@constraint(m, h[pl,t̂] == ngLine_data[pl].H_ini + q_in[pl,t̂] - q_out[pl,t̂] + slack[pl,t̂])
                @constraint(m, h[pl,t̂] == ngLine_data[pl].H_ini + q_in[pl,t̂] - q_out[pl,t̂])
            end
            if t̂ != 1 #All hours other than first
                #@constraint(m, h[pl,t̂] == h[pl,t̂-1] + q_in[pl,t̂] - q_out[pl,t̂] + slack[pl,t̂])
    ...
    println(get_status_code(m))

    #println(wm_soc[:,1])
    @info("DRCC Model M1 status ---> $(status)")
...
#uncomment if solving ng+el system
(status,cost, elflows, elprod, windgen, vangs, ng_q_inflows, ng_q_outflows, ng_q_flows, linepack_amount, ng_pre, ng_prod) = unidir_DRCC_McCormick_SOCP_EL_NG()
#Uncomment if solving only el system
#(status,cost,el_lmp, elflows, elprod, windgen, vangs) = unidir_DRCC_McCormick_SOCP_EL_NG()
println(cost)
